...
const mondayFetch = async (query, apiVersion = "2024-01") => {
    const mondayResponse = await fetch(
        `https://monday.weuit.com/monday`,
        {
            ...
            headers: {
                'Content-Type': 'application/json',
                'Authorization': 'eyJhbGciOiJIUzI1NiJ9.eyJ0aWQiOjYwNjM1NDk2OCwiYWFpIjoxMSwidWlkIjozMDI3MzE5NCwiaWFkIjoiMjAyNi0wMS0xMlQxNDo1NDo1NC4wMDBaIiwicGVyIjoibWU6d3JpdGUiLCJhY3RpZCI6ODg0NzExMCwicmduIjoidXNlMSJ9.Ktk1F5RanMKS6NxEmd-59G_wg64Ab5lEt2O-6U2lNt0',
                'API-Version': apiVersion
            },
            body: JSON.stringify({ query })
        }
...
const fixedData = {
    ...
    urls: {
        ...
        baseUrl: "https://dmsprd.laserappraiserservices.com/appraisals",
        appraiseUrl: "https://dmsprd.laserappraiserservices.com/vehicle-detail"
    },
    errorNotices: {
    ...
    switch (position) {
            ...
            if (mondayItem.local.step == null || mondayItem.local.step == 'verifyAppraiser') {
                ...
                if (currentUrl.includes(fixedData.urls.baseUrl)) {
                    ...
                    if (loginInput != null || passwordInput != null) {
                        ...
                        const laserIdUrl = document.querySelector("a[href*='deviceIdentifier']");
                        if (laserIdUrl != null) {
                            const laserId = new URL(laserIdUrl.href).searchParams.get('deviceIdentifier');
                            if (currentUrl == fixedData.urls.baseUrl) {
                                const appraiser = document.querySelector('nav header button[data-slot="trigger"] svg[viewBox="0 0 448 512"]')?.parentElement || {};
        ...
        case 'suggestItem':
            ...
            const appraisalResult = async (info) => {
                ...
                // }
                await sleep(10000);
                // const kbb = document.querySelector("td#kbb_trade_xclt_adj");//data-key="AdjustedAdjustedexcellent"
                const kbb = document.querySelector("#KBB")?.parentElement?.querySelectorAll('[data-key="AdjustedAdjustedexcellent"]')[1]
                // const jd = document.querySelector("td#nada_retail_rtl_adj");//AdjustedAdjustedretail
                const jd = document.querySelector("#NADA")?.parentElement?.querySelector('[data-key="AdjustedAdjustedretail"]');
                // const kbbRetail = document.querySelector("td#kbb_misc_retail_adj");//AdjustedAdjustedRetail
            ...
            if (mondayItem.local.step == 'suggestItem') {
                ...
                const currentUrl = window.location.href;
                // https://dmsprd.laserappraiserservices.com/vehicle-detail?vin=1C6RREJT4LN160836&deviceIdentifier=1DF10B81-E746-4E47-B0EB-D2ABF43C49C6&mileage=65456
                // url.searchParams.set('pageAction','vinNew');
                url.searchParams.set('deviceIdentifier', mondayItem.local.laserId);
                ...
                if (currentUrl != url.href) {
                    ...
                    // https://dmsprd.laserappraiserservices.com/vehicle-detail?deviceIdentifier=1DF10B81-E746-4E47-B0EB-D2ABF43C49C6&vin=2C4RC1BG3HR661392&mileage=114500
                    // https://dmsprd.laserappraiserservices.com/vehicle-detail?vin=2C4RC1BG3HR661392&deviceIdentifier=1DF10B81-E746-4E47-B0EB-D2ABF43C49C6&mileage=114500
                    const postAppraise = await fetch("https://dmsprd.laserappraiserservices.com/appraisals", {
                        "headers": {
                            "accept": "text/x-component",
                            "accept-language": "en-US,en;q=0.9",
                            "content-type": "text/plain;charset=UTF-8",
                            "next-action": "034490e83eec9680e2a3a24c763a634f78d5dbda",
                            "next-router-state-tree": "%5B%22%22%2C%7B%22children%22%3A%5B%22appraisals%22%2C%7B%22children%22%3A%5B%22__PAGE__%22%2C%7B%7D%2C%22%2Fappraisals%22%2C%22refresh%22%5D%2C%22modal%22%3A%5B%22__DEFAULT__%22%2C%7B%7D%5D%7D%5D%7D%2Cnull%2Cnull%2Ctrue%5D",
                            "priority": "u=1, i",
                        ...
                        "referrer": "https://dmsprd.laserappraiserservices.com/appraisals",
                        "referrerPolicy": "strict-origin-when-cross-origin",
                        body: JSON.stringify([
                            {
                                deviceIdentifier: mondayItem.local.laserId,
                                vin: mondayItem.local.item.vin.toUpperCase(),
                                isDrillDown: false,
                                mileage: mondayItem.local.item.mileage
                            }
                        ]),
                    ...
                    if (!postAppraise.ok) {
                        ...
                        throw new Error(`Post Appraise failed with status ${postAppraise.status}`);
                    }
                    // await postAppraise.text();
                    window.location.href = url.href;
                    console.log(("post request to appraise"))
                } else {
                    // await sleep(5000);
